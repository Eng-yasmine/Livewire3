...
<!-- أزرار إنشاء المهام وإدارة التصنيفات -->
        ...
        @foreach($taskCategories as $taskCategory)
            <li class="w-full pr-2 border-b border-gray-200 sm:border-b-0 sm:border-r dark:border-gray-600">
                <div class="flex items-center ps-3">
                    <input wire:model.live="selectedTaskCategories" id="{{ $taskCategory->id }}-checkbox-list" type="checkbox" value="{{ $taskCategory->id }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500">
                    <label for="{{ $taskCategory->id }}-checkbox-list" class="w-full py-3 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">{{ $taskCategory->name }}</label>
                </div>
            </li>
        @endforeach
    </ul>
                ...
                <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700 border-gray-200">
                    ...
                    <td class="px-6 py-4 space-x-1">
                        @foreach($task->taskCategories as $taskCategory)
                            <span class="rounded-full bg-gray-200 px-2 py-1 text-xs dark:bg-gray-600 dark:text-white">
                                {{ $taskCategory->name }}
                            </span>
                        @endforeach
                    </td>
                    ...
                    <td class="px-6 py-4 space-x-2">
                        ...
                        <flux:button wire:confirm="Are you sure you want to delete this task?" wire:click="deleteTask({{ $task->id }})" variant="danger" type="button">{{ __('Delete') }}</flux:button>
                    </td>
                </tr>
            @endforeach
        </tbody>
